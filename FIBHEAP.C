...
void intertion(struct FIB_HEAP *H,struct NODE *new,int val);
...
heap=(struct FIB_HEAP *)malloc(sizeof(FIB_HEAP));
heap=NULL;
while(1)
{
printf("\n Operations\n1.Create fibonacci heap\n2.Insert nodes into fibonacciheap\n3.find min\n4.print\n<press any key to exit");
...
case 2:if(heap==NULL)
	...
	{
	printf("Node %d and the key value=",i);
	scanf("%d",&ele);
...
case 3: min_node=find_min_node(heap);
	...
	printf("\n Minimum value=%d",min_node->key);
	break;
case 4:print_heap(heap->min);
	break;
